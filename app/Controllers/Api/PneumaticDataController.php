        ...
        if (!$device) {
            ...
            if($this->validate($rules)){
                try {
                    ...
                    if($data){
                        $pneumatic_pair->update($data['id'],['final_status'		=>  1, 'final_test'		=>  date("Y-m-d H:i:s", now("Asia/Colombo"))]);
                        $ok = 1;
                        ...
                        $rrfid_entry = $pneumatic_pair->where('left_rfid', $this->request->getVar('RRFID'))->orwhere('right_rfid', $this->request->getVar('RRFID'))->first();
                        if($lrfid_entry){
                            $pneumatic_pair->update($lrfid_entry['id'],['final_status'		=>  2, 'final_test'		=>  date("Y-m-d H:i:s", now("Asia/Colombo"))]);
                        }
                        if($rrfid_entry){
                            $pneumatic_pair->update($rrfid_entry['id'],['final_status'		=>  2, 'final_test'		=>  date("Y-m-d H:i:s", now("Asia/Colombo"))]);
                        }
                        $ok = 0;
                        $message = 'pair didnt match';
                    }
